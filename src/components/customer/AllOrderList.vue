...
export default {
    ...
    mounted() {
        const that = this
        axios.get('/customer/getAllOrder?customerId=' + localStorage.getItem("customerId"))
            .then(function (res) {
                ...
                for (let i = 0; i < that.tableData.length; i++) {
                    if (that.tableData[i].payState === 1) {
                        that.tableData[i].payState = '已支付'
                    } else if (that.tableData[i].payState === 0) {
                        that.tableData[i].payState = '未支付'
                    }
                    if (that.tableData[i].state === 2 && that.tableData[i].payState === 1) {
                        that.tableData[i].payState = '已退款'
                    }
                }
            })
    }
