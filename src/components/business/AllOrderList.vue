...
export default {
    ...
    mounted() {
        const that = this
        axios.get('/shop/getAllOrder?shopId=' + localStorage.getItem("shopId"))
            .then(function (res) {
                ...
                for (let i = 0; i < that.tableData.length; i++) {
                    if (that.tableData[i].state === 1 && that.tableData[i].payState === 1) {
                        that.tableData[i].state = '已确认'
                    } else if (that.tableData[i].state === 0 && that.tableData[i].payState === 1) {
                        that.tableData[i].state = '未确认'
                    } else if (that.tableData[i].state === 2) {
                        that.tableData[i].state = '已退款'
                    } else {
                        that.tableData[i].state = '未支付'
                    }
                }
            })
    }
